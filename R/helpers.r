...
nc.get.dim.axes.from.names <- function(f, v, dim.names) {
  ...
  return(sapply(dim.names, function(x, y) { ifelse(any(tolower(x) == tolower(names(y))), y[tolower(x) == tolower(names(y))], NA) }, c("lat"="Y", "latitude"="Y", "lon"="X", "longitude"="X", "xc"="X", "yc"="Y", "x"="X", "y"="Y", "t"="T", "date"="T", "datetime"="T", "d"="T", "time"="T", "timeofyear"="T", "plev"="Z", "lev"="Z", "level"="Z")))
}
